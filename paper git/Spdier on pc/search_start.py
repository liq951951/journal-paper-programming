...
from hour_slice import hour_slice
...
def fetch_pages(keyword, start_time, end_time):
    ...
    for i in range(len(mblogs)):
        cursor = conn.cursor()
        sql = "insert into ncrk(wb_id,wb_username,wb_userid,wb_content,wb_createtime,wb_forwardnum,wb_commentnum,wb_likenum,wb_url) values(%s,%s,%s,%s,%s,%s,%s,%s,%s)"
        cursor.execute(sql,(mblogs[i]['wb_id'], mblogs[i]['wb_username'], mblogs[i]['wb_userid'],
                            ...
                            mblogs[i]['wb_commentnum'], mblogs[i]['wb_likenum'],mblogs[i]['wb_url']))
        print('************** %s  数据保存成功 **************' )
        conn.commit()
        cursor.close()
        return mblogs
...
if __name__ == '__main__':
    keyword = "农村人口"
    start_time = "2019.12.31.0"
    end_time = "2019.12.31.24"
